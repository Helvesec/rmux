...
```bash
tools/bench/run-unix.sh --out target/benchmarks/linux.json
tools/bench/run-windows.ps1 -Out target/benchmarks/windows.json
tools/bench/run-unix.sh --out target/benchmarks/macos.json
python3 tools/bench/render.py target/benchmarks/*.json --output docs/benchmarks.md
```
...
def markdown(payloads: list[dict[str, Any]], asset_dir: Path, summary_rows: int) -> str:
    ...
    return f"""<!-- Generated by tools/bench/render.py; edit the renderer or benchmark JSON inputs. -->
<div align="center">
