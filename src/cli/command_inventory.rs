...
const LIST_COMMAND_SIGNATURES: &[(&str, &str)] = &[
    ...
    (
        "web-share",
        "[-lwX] [-K share-id] [--config] [--lookup share-id] [--ttl seconds] [--max-readers count] [--frontend-url url] [--tunnel-url url] [--no-navbar] [--no-disclaimer] [--theme user|light|dark] [--pin] [-t target-pane]",
    ),
];
