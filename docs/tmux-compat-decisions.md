...
| `tmux -CC new-session -A -- cmd` streams child stdout as `%output` frames | `ISO required` | Known gap: RMUX control mode does not reliably emit the command output frames | deferred control-mode integration test |
